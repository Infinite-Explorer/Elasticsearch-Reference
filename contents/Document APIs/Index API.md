...
在使用索引API更新文档的时候总是会创建一个新版本的文档，即使该文档没有修改过。如果不想这样，可以通过使用`_update`API，并将`detect_noop`选项设为true。该选项在索引API终不可用的原因是索引API不会获取旧文档及将其与新文档进行比较。

没有强行规定什么时候不能使用空更新。这取决于很多因素，比如你的数据源空更新的频率是否频繁，跑在接收空更新的分片上ES每秒钟有多少查询请求。
